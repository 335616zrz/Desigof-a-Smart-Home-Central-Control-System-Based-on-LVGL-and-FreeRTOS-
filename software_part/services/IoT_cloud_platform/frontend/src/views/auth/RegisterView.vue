<template>
  <div class="register-page-wrapper">
    <div class="scene">
      ...
      <div class="layout">
        <div :class="['panel', 'panel-left', { shake }]">
          <div class="logo-row">
          ...
          <h1>创建账户</h1>
          <p class="desc">注册新用户以使用 IoT 云平台。</p>
          ...
          <form class="register-form" @submit.prevent>
            ...
            <label class="field">
              ...
              <input
                ...
                @input="handleInput"
              />
              <div class="helper">{{ helpers.email }}</div>
            </label>
            ...
            <label class="field">
              <span class="label"><span class="required">*</span> 确认密码</span>
              <div class="input-wrap">
                <input
                  ...
                  @input="handleInput"
                />
                <span class="eye" aria-hidden="true" @click="showConfirmPassword = !showConfirmPassword" :title="showConfirmPassword ? '隐藏密码' : '显示密码'">
              ...
              <div class="helper">{{ helpers.confirmPassword }}</div>
            </label>
...
const helpers = reactive({ username: '', email: '', password: '', confirmPassword: '' });
...
const updateHelpers = () => {
  ...
  helpers.email = e.length > 0 && !emailRegex.test(e) ? '邮箱格式不正确' : '';
  helpers.password = p.length === 0 ? '请输入密码' : p.length < 6 ? '至少 6 位' : '';
  helpers.confirmPassword = c.length === 0 ? '请再次输入密码' : c !== p ? '两次密码不一致' : '';
};
...
const handleRegister = async () => {
  ...
  const eOk = form.email.trim().length === 0 || emailRegex.test(form.email.trim());
  const pOk = form.password.trim().length >= 6;
  const cOk = form.confirmPassword.trim() === form.password.trim() && form.confirmPassword.trim().length >= 6;
  ...
  try {
    await auth.register(form.username, form.email || undefined, form.password);
    handleSuccess();
...
onBeforeUnmount(() => {
  [helperTimerId, shakeTimerId, pulseTimerId].forEach((id) => {
...
<style>
/* RegisterView 独立的全屏包装器，与 LoginView 保持一致 */
