...
try {
  if (typeof globalThis !== 'undefined') {
    // Keep both names to avoid conflicts with other libs and ease debugging.
    if (!globalThis.echarts) globalThis.echarts = echarts;
    globalThis.__ec_echarts__ = echarts;
  }
} catch (e) {}
...
Component({
  ...
  ready: function () {
    ...
    // In some uni-app X / setData interop cases, `ec` can be dropped (especially when it contains
    // function fields like `onInit`). Keep a usable default and continue init so the parent can
    // handle the `init` event fallback.
    if (!this.data.ec || Object.keys(this.data.ec).length === 0) {
  ...
  methods: {
    ...
    init: function (callback, retryCount = 0) {
      ...
      // Devtools sometimes reports SDKVersion like "2.01.xxxx" which would fail the semver check
      // even though it supports <canvas type="2d">. Force-enable the new canvas in devtools.
      const canUseNewCanvas = platform === 'devtools' || compareVersion(version, '2.9.0') >= 0;
      const forceUseOldCanvas = this.data.forceUseOldCanvas;
      const isUseNewCanvas = canUseNewCanvas && !forceUseOldCanvas;
      this.setData({ isUseNewCanvas });

      if (DEBUG_EC_CANVAS) {
        console.log('[ec-canvas] init', {
          canvasId: this.data.canvasId,
          platform,
          SDKVersion: version,
          isUseNewCanvas
