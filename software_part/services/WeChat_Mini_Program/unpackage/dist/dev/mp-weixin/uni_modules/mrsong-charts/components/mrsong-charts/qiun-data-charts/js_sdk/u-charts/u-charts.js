...
function calCategoriesData(categories, opts, config2, eachSpacing, context) {
  ...
  var fontSize = opts.xAxis.fontSize * opts.pix || config2.fontSize;
  ...
  if (opts.xAxis.rotateLabel == true) {
    ...
    let tempHeight = 2 * config2.xAxisTextPadding + Math.abs(maxTextLength * Math.sin(result.angle));
    tempHeight = tempHeight < fontSize + 2 * config2.xAxisTextPadding ? tempHeight + 2 * config2.xAxisTextPadding : tempHeight;
...
function calXAxisData(series, opts, config2, context) {
  ...
  var textLength = xAxisScaleValues.map(function(item) {
    return measureText(item, opts.xAxis.fontSize * opts.pix || config2.fontSize, context);
  });
  var maxTextLength = Math.max.apply(this, textLength);
  if (maxTextLength + 2 * config2.xAxisTextPadding > eachSpacing) {
...
function drawMountPointText(points, series, config2, context, opts) {
  series.data;
  var textOffset = series.textOffset ? series.textOffset : 0;
  points.forEach(function(item, index) {
    if (item !== null) {
...
function drawMarkLine(opts, config2, context) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (item.showLabel) {
      ...
      context.setFontSize(config2.fontSize);
      let textWidth = measureText(labelText, config2.fontSize, context);
      let bgWidth = textWidth + config2.toolTipPadding * 2;
      let bgStartX = item.labelAlign == "left" ? opts.area[3] - bgWidth : opts.width - opts.area[1];
      bgStartX += item.labelOffsetX;
      let bgStartY = item.y - 0.5 * config2.fontSize - config2.toolTipPadding;
      bgStartY += item.labelOffsetY;
      let textX = bgStartX + config2.toolTipPadding;
      item.y;
      context.setFillStyle(hexToRgb(item.labelBgColor, item.labelBgOpacity));
      context.setStrokeStyle(item.labelBgColor);
      context.setLineWidth(1 * opts.pix);
      context.beginPath();
      context.rect(bgStartX, bgStartY, bgWidth, config2.fontSize + 2 * config2.toolTipPadding);
      context.closePath();
      context.stroke();
      context.fill();
      context.setFontSize(config2.fontSize);
      ...
      context.fillText(String(labelText), textX, bgStartY + config2.fontSize + config2.toolTipPadding / 2);
      context.stroke();
      context.setTextAlign("left");
    }
  }
...
function drawToolTipHorizentalLine(opts, config2, context, eachSpacing, xAxisPoints) {
  ...
  if (toolTipOption.yAxisLabel) {
    ...
    for (let i = 0; i < labelText.length; i++) {
      context.setFontSize(config2.fontSize);
      let textWidth = measureText(labelText[i], config2.fontSize, context);
      let bgStartX, bgEndX, bgWidth;
      if (widthArr[i].position == "left") {
        bgStartX = tStartLeft - widthArr[i].width;
        bgEndX = Math.max(bgStartX, bgStartX + textWidth + config2.toolTipPadding * 2);
      } else {
        bgStartX = tStartRight;
        bgEndX = Math.max(bgStartX + widthArr[i].width, bgStartX + textWidth + config2.toolTipPadding * 2);
      }
...
function drawToolTip(textList, offset, opts, config2, context, eachSpacing, xAxisPoints) {
  ...
  var textWidth = textList.map(function(item) {
    return measureText(item.text, config2.fontSize, context);
  });
  ...
  textList.forEach(function(item, index) {
    if (item.color !== null) {
      ...
      var startY = offset.y + (config2.toolTipLineHeight - config2.fontSize) / 2 + config2.toolTipLineHeight * index + config2.toolTipPadding + 1;
      if (isOverRightBorder) {
  ...
  textList.forEach(function(item, index) {
    ...
    var startY = offset.y + (config2.toolTipLineHeight - config2.fontSize) / 2 + config2.toolTipLineHeight * index + config2.toolTipPadding;
    context.beginPath();
    context.setFontSize(config2.fontSize);
...
function drawMountDataPoints(series, opts, config2, context) {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (item !== null && i > leftNum && i < rightNum) {
          ...
          if (mountOption.barBorderRadius && mountOption.barBorderRadius.length === 4 || mountOption.barBorderCircle === true) {
            ...
            const top = item.y;
            const width = item.width;
            const height2 = opts.height - opts.area[2] - item.y - mountOption.borderWidth * opts.pix / 2;
  ...
  if (opts.dataLabel !== false && process === 1) {
    ...
    var points = getMountDataPoints(series, minRange2, maxRange2, xAxisPoints, eachSpacing, opts, mountOption, process);
    drawMountPointText(points, series, config2, context, opts);
  }
  context.restore();
  return {
    xAxisPoints,
...
function drawBubbleDataPoints(series, opts, config2, context) {
  ...
  series.forEach(function(eachSeries, seriesIndex) {
    ...
    if (opts.dataLabel !== false && process === 1) {
      points.forEach(function(item, index) {
        context.beginPath();
        var fontSize = series.textSize * opts.pix || config2.fontSize;
        context.setFontSize(fontSize);
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config2._xAxisTextAngle_ === 0) {
      newCategories.forEach(function(item, index) {
        ...
        context.fillText(String(xitem), xAxisPoints[index] + offset, startY + xAxisFontSize + (config2.xAxisHeight - scrollHeight - xAxisFontSize) / 2);
        context.closePath();
        context.stroke();
      });
    } else {
      newCategories.forEach(function(item, index) {
        ...
        var offsetY = startY + 6 * opts.pix + xAxisFontSize - xAxisFontSize * Math.abs(Math.sin(config2._xAxisTextAngle_));
...
uCharts.prototype.zoom = function() {
  ...
  if (this.opts.enableScroll !== true) {
    common_vendor.index.__f__("log", "at uni_modules/mrsong-charts/components/mrsong-charts/qiun-data-charts/js_sdk/u-charts/u-charts.js:6790", "[uCharts] 请启用滚动条后使用");
    return;
  }
...
uCharts.prototype.dobuleZoom = function(e) {
  if (this.opts.enableScroll !== true) {
    common_vendor.index.__f__("log", "at uni_modules/mrsong-charts/components/mrsong-charts/qiun-data-charts/js_sdk/u-charts/u-charts.js:6830", "[uCharts] 请启用滚动条后使用");
    return;
  }
...
uCharts.prototype.showToolTip = function(e) {
  ...
  if (!touches) {
    common_vendor.index.__f__("log", "at uni_modules/mrsong-charts/components/mrsong-charts/qiun-data-charts/js_sdk/u-charts/u-charts.js:7008", "[uCharts] 未获取到event坐标信息");
  }
...
//# sourceMappingURL=../../../../../../../../.sourcemap/mp-weixin/uni_modules/mrsong-charts/components/mrsong-charts/qiun-data-charts/js_sdk/u-charts/u-charts.js.map
